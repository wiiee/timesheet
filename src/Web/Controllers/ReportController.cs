    ...
    public class ReportController : BaseController
    {
        ...
        private List<FlightWeeklyReportModel> BuildFlightWeeklyReportModel(DateTime startDate, DateTime endDate, bool isShowDetails)
        {
            ...
            if (userGroups.Count > 0)
            {
                ...
                sortedWeeklyReportModels = models.Where(o => o.UserGroupName != "深圳机票测试组").OrderBy(p => p.UserGroupName).ThenBy(p => p.ActualEndDate).ToList();
                sortedTestWeeklyReportModels = models.Where(o => o.UserGroupName == "深圳机票测试组").OrderBy(p => p.UserGroupName).ThenBy(p => p.ActualEndDate).ToList();
                sortedWeeklyReportModels.AddRange(sortedTestWeeklyReportModels.Where(o => !sortedWeeklyReportModels.Contains(o)));
                models = sortedWeeklyReportModels.OrderByDescending(o => o.Project.Level).ThenBy(o => o.Project.GetEndDate()).ToList();
            }
